...
const doc={getElementById:id=>elements[id],createElement:tag=>new Element(tag),createElementNS:(_,tag)=>new Element(tag),addEventListener(){},querySelectorAll:selector=>selector==='[data-mode]'?modeButtons:selector==='[data-code]'?descendants().filter(n=>n.attrs['data-code']):selector==='.paddle'?[elements.dit,elements.dah]:[]};
...
let run;context.capture=fn=>{run=fn;};
const source=fs.readFileSync(__dirname+'/app.js','utf8');
// Test-only closure access; production exposes no globals.
vm.runInContext(source.replace(/\}\)\(\);\s*$/, 'capture(code=>eval(code));})();'),context);
...
for(const [char,code] of Object.entries(codes)){run('reset()');for(const s of code)pulse(s);run('commitChar()');assert.equal(run('transcript'),char,char+' decoding');}
