...
window.TECH_ARTICLES = [
  ...
  },
  {
    ...
    "category": "ツール",
    "date": "2026-08-29",
    "description": "公開している自作 Web ツールの一覧。分岐図とホバーパドルで英数字を練習する Morse Atelier、VRM モデルにポーズを付けて作画資料にする VRM Pose Ref、書き順で戦う漢字学習ゲーム 漢字サモナーズ（試験公開）、CG の数学を数式なし・コード表記で学ぶ教材 CGの数学を、数式なしで。、その続編としてゲームのリアルタイム VFX をエンジン非依存の原則と動く図で学ぶ ゲームVFXの原則を、動く図で。など。すべて端末内で動作し、モデルやデータは外部に送信されません。"
  },
  {
